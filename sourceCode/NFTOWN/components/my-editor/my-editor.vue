<template>
  ...
  <view class='wrapper'>
    <view class='toolbar' @tap="format">
      ...
      <view :class="formats.color === '#49b3f5' ? 'ql-active' : ''" class="iconfont icon-text_color" data-name="color"
        data-value="#49b3f5"></view>
      <view :class="formats.backgroundColor === '#6effc3' ? 'ql-active' : ''" class="iconfont icon-fontbgcolor"
        data-name="backgroundColor" data-value="#6effc3"></view>
    ...
    <editor id="editor" class="ql-container" placeholder="开始输入..." showImgSize showImgToolbar showImgResize
      @statuschange="onStatusChange" :read-only="readOnly" @ready="onEditorReady" @blur="getHTML">
    </editor>
  </view>
</template>
...
<script>
  export default {
    ...
    methods: {
      getHTML(e) {
        // this.htmlHere = e.detail.html
        uni.$emit('upload', e.detail.html);
      },
      readOnlyChange() {
